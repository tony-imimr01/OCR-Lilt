...
FIXED: Missing answer entities and question-answer linking
- Added proper question-answer entity detection with lower confidence thresholds
- Implemented question-answer linking based on spatial proximity
- Fixed FUNSD format compliance with required fields
- Enhanced entity post-processing for complete question-answer pairs
- Added fallback logic for missing entity types
- FIXED: Increased max entities limit to 350 and improved fallback logic
Usage:
...
class LiLTConfig:
    def __init__(self, args=None):
        ...
        # Processing - FIXED: Lowered confidence thresholds, especially for answers
        self.normalize_coordinates = True
        self.coordinate_range = 1000
        self.min_confidence = 0.2  # Lowered from 0.3
        self.min_confidence_per_type = {
            EntityTypes.HEADER: 0.25,
            EntityTypes.QUESTION: 0.20,
            EntityTypes.ANSWER: 0.15,  # Significantly lower for answers
            EntityTypes.OTHER: 0.10
        }
        self.max_entities_per_doc = 350  # FIXED: Increased from 200 to 350
        self.min_entity_length = 2
        # Question-Answer linking
        self.enable_qa_linking = True
        self.max_qa_distance = 200  # Maximum distance between question and answer
        ...
        # Default label (safe fallback to avoid missing-labels)
        self.default_label = 0
        # FIXED: Complete entity mapping with proper answer support
...
def setup_tesseract() -> bool:
    try:
        ...
        logger.error("Please install Tesseract OCR or ensure it's in PATH.")
        return False
...
def load_lilt_model(config: LiLTConfig, device: torch.device):
    ...
    try:
        ...
        # FIXED: Use complete entity mapping with proper answer support
        ...
        logger.info(f"Label mapping: {model.config.label2id}")
...
def compute_union_box(boxes: List[List[int]]) -> List[int]:
    """Compute the union bounding box for multiple boxes with improved accuracy"""
    if not boxes:
        return [0, 0, 0, 0]
    ...
    # Ensure valid dimensions - especially important for short answer entities
    x2 = max(x2, x1 + 1)
    y2 = max(y2, y1 + 1)
    return [x1, y1, x2, y2]
def clean_text(text: str) -> str:
    """Clean and normalize text with special handling for answer entities"""
    if not isinstance(text, str):
        return ""
    text = re.sub(r"\s+", " ", text).strip()
    text = ''.join(c for c in text if c.isprintable() or c in ['\n', '\t'])
    text = text.replace('ﬁ', 'fi').replace('ﬂ', 'fl').replace('ﬀ', 'ff')  # Fix common OCR artifacts
    # Special cleanup for numeric answers
...
def validate_bbox(bbox: List[int]) -> bool:
    ...
    try:
        ...
        width = x2 - x1
        height = y2 - y1
        return (x1 >= 0 and y1 >= 0 and x2 > x1 and y2 > y1 and 
                width >= 1 and height >= 1 and x2 <= 20000 and y2 <= 20000)
    except (ValueError, TypeError):
        return False
def get_entity_type_from_label(label: str) -> str:
    """Extract entity type from BIO label (e.g., 'B-QUESTION' -> 'question')"""
...
def validate_entity(entity: Dict) -> bool:
    ...
    if not validate_bbox(entity['box']):
        return False
    # Special validation for answer entities - allow very short text
    if entity['label'] == EntityTypes.ANSWER and len(entity.get('text', '').strip()) < 1:
        return False
    if not entity.get('text', '').strip() and entity['label'] not in [EntityTypes.ANSWER, EntityTypes.FALLBACK]:
        return False
    return True
# ---------- Question-Answer Linking ----------
def link_questions_and_answers(entities: List[Dict], width: int, height: int) -> List[Dict]:
    """Link questions with their corresponding answers based on spatial proximity and document structure"""
    questions = [e for e in entities if e['label'] == EntityTypes.QUESTION]
    answers = [e for e in entities if e['label'] == EntityTypes.ANSWER]
    ...
    for question in questions:
        ...
        for answer in answers:
            ...
            # Prefer answers that are horizontally aligned or slightly to the right
            ...
            if ':' in question['text'] or '?' in question['text']:
                text_score += 0.3
            if re.match(r'^[\d\.,$€£\-/]+$', answer['text'].strip()):
                text_score += 0.2
        ...
        if best_answer:
            ...
            logger.debug(f"Linked question '{question['text']}' with answer '{best_answer['text']}' (score: {best_score:.2f})")
    return entities
# ---------- Augmentation ----------
...
# ---------- Entity extraction (FIXED FOR QUESTION-ANSWER PAIRS) ----------
def extract_entities(model, tokenizer, image_path: Path, config: LiLTConfig, device: torch.device) -> Optional[Dict]:
    ...
    if not words:
        try:
            # Preprocess image for better OCR - especially important for answer text
            gray = image.convert('L')
            # Enhance contrast more aggressively for better number/date recognition
            ...
            ocr_data = pytesseract.image_to_data(
                ...
                config='--psm 6 --oem 1'  # More aggressive OCR engine mode
            )
            ...
            for i in range(n_boxes):
                ...
                try:
                    ...
                    conf = -1.0
                # Lower confidence threshold for potential answer text (numbers, dates)
                if re.match(r'^[\d\-/\\.,]+$', text):
                    min_conf_needed = config.min_confidence * 0.3  # Much lower for numbers
                else:
                    min_conf_needed = config.min_confidence * 0.7
    ...
    if not words:
        ...
        # FIXED: Create fallback entities even when no OCR text is found
        logger.info(f"Creating fallback entities for {image_path.name} due to no OCR text")
        return {
            "entities": [{
    ...
    if not words:
        ...
        # FIXED: Create fallback entities even when no valid text after cleaning
        logger.info(f"Creating fallback entities for {image_path.name} due to no valid text")
        return {
            "entities": [{
                "box": [0, 0, config.coordinate_range, config.coordinate_range],
    ...
    # Tokenize words (split into tokens) and align boxes
    try:
        encoding = tokenizer(
            words,
        ...
        # Align boxes to tokens - FIXED: Proper word ID handling
        try:
        ...
        logger.debug(f"Word IDs sample: {word_ids[:10]}")
        logger.debug(f"Aligned boxes sample: {aligned_boxes[:5]}")
    except Exception as e:
    ...
    try:
        ...
        logger.debug(f"Prediction counts: {[pred_labels.count(label) for label in unique_preds]}")
    except Exception as e:
        logger.error(f"Model inference error for {image_path}: {e}")
        logger.debug(f"Input shapes - input_ids: {encoding['input_ids'].shape}, bbox: {bbox_tensor.shape}")
        logger.exception(e)
        return None
    # FIXED: Enhanced entity post-processing with proper B/I handling for questions and answers
    entities = []
    current_entity = None
    entity_id = 0
    ...
    for idx in range(len(predictions)):
        ...
        if token in [tokenizer.cls_token, tokenizer.sep_token, tokenizer.pad_token, "[PAD]", "[CLS]", "[SEP]"]:
            continue
        # Skip already processed tokens (for subword tokens)
        ...
        if label == "O" or prefix == "O":
            if current_entity:
                ...
                entity_text = clean_text(current_entity["text"].strip())
        ...
        # Handle I (inside) labels
            ...
            if current_entity and current_entity["label"] == entity_type:
                ...
                # Start new entity if no current entity or type mismatch
                if current_entity:
    ...
    # FIXED: Ensure we have at least one of each entity type if text exists
    entity_types_present = set(e['label'] for e in entities)
    logger.info(f"Entity types found: {entity_types_present}")
    # FIXED: Enhanced fallback logic for missing question/answer entities
    has_questions = any(e['label'] == EntityTypes.QUESTION for e in entities)
    has_answers = any(e['label'] == EntityTypes.ANSWER for e in entities)
    ...
    if not has_questions or not has_answers:
        ...
        # Strategy 1: Use text patterns to identify questions and answers
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            chunk_clean = clean_text(chunk.strip())
            if not chunk_clean:
                continue
            ...
            is_question = (
                ...
                chunk_clean.endswith('?') or
                len(chunk_clean.split()) <= 6 or  # Short text is likely a label/question
                any(keyword in chunk_clean.lower() for keyword in ['name', 'date', 'address', 'phone', 'email', 'total', 'amount', 'id', 'number'])
            )
            
            # Create entity based on heuristics
            entity_label = EntityTypes.QUESTION if is_question else EntityTypes.ANSWER
            
            # Only create the missing entity type
            if (entity_label == EntityTypes.QUESTION and not has_questions) or (entity_label == EntityTypes.ANSWER and not has_answers):
                # Create a reasonable bounding box (distribute vertically)
        ...
        # Strategy 2: If still missing entities, create minimal fallback entities
        if not has_questions and not any(e['label'] == EntityTypes.QUESTION for e in fallback_entities):
            fallback_entities.append({
                "box": [0, 0, config.coordinate_range, int(config.coordinate_range * 0.1)],
                "text": "Question field",
                "label": EntityTypes.QUESTION,
                "words": [{"box": [0, 0, config.coordinate_range, int(config.coordinate_range * 0.1)], "text": "Question field"}],
                "linking": [],
                "id": len(entities) + len(fallback_entities)
            })
            
        if not has_answers and not any(e['label'] == EntityTypes.ANSWER for e in fallback_entities):
            fallback_entities.append({
                "box": [0, int(config.coordinate_range * 0.15), config.coordinate_range, int(config.coordinate_range * 0.25)],
                "text": "Answer field",
                "label": EntityTypes.ANSWER,
                "words": [{"box": [0, int(config.coordinate_range * 0.15), config.coordinate_range, int(config.coordinate_range * 0.25)], "text": "Answer field"}],
                "linking": [],
                "id": len(entities) + len(fallback_entities)
            })
        
        # Add fallback entities to the main list
        entities.extend(fallback_entities)
        logger.info(f"Added {len(fallback_entities)} fallback entities: {', '.join(e['label'] for e in fallback_entities)}")
    # FIXED: Filter and validate entities with per-type confidence
    valid_entities = []
    for entity in entities:
        if validate_entity(entity):
            # Apply confidence threshold per entity type - be more lenient with answers
            min_conf = config.get_min_confidence_for_entity(entity['label'])
            # Special handling for short answer entities (dates, numbers, etc.)
            if entity['label'] == EntityTypes.ANSWER and len(entity['text'].strip()) <= 3:
                min_conf = min(0.1, min_conf)  # Very low threshold for short answers
    ...
    if len(valid_entities) > config.max_entities_per_doc:
        ...
        # Sort by text length and confidence, keep the most important ones
        valid_entities.sort(key=lambda x: (len(x['text']), x['label'] in [EntityTypes.QUESTION, EntityTypes.ANSWER]), reverse=True)
        valid_entities = valid_entities[:config.max_entities_per_doc]
    # FIXED: Link questions and answers
    if config.enable_qa_linking:
        valid_entities = link_questions_and_answers(valid_entities, width, height)
    ...
    logger.info(f"Question entities: {question_count}, Answer entities: {answer_count}")
    if valid_entities:
...
def create_funsd_document(doc_info: Dict[str, Any], config: LiLTConfig, label: Optional[Union[int, str]] = None) -> Dict[str, Any]:
    ...
    for entity in doc_info["entities"]:
        ...
        # Add words with proper structure
    ...
    # FUNSD format structure - FIXED to match expected format
    document = {
        "id": f"en_custom_{unique_id}",
        "uid": unique_id,
        "file": doc_info["filename"],  # Required field for FUNSD format
        "img": {
            "fname": doc_info["filename"],
            "width": doc_info["width"],
            "height": doc_info["height"]
        },
        "document": entities,  # Must be a list of entities
        "label": label
    }
...
def create_funsd_json(doc_info: Dict[str, Any], config: LiLTConfig, label: Optional[Union[int, str]] = None) -> Dict[str, Any]:
    """Create complete FUNSD-format JSON with proper question-answer structure"""
    document = create_funsd_document(doc_info, config, label=label)
    ...
    for entity in document["document"]:
        ...
        if "words" not in entity or not entity["words"]:
            # Create minimal word structure
            entity["words"] = [{
                "box": entity["box"],
                "text": entity["text"]
            }]
        # Ensure linking is properly formatted
    ...
    return {
        ...
        "split": "custom",
        "label": top_label,
        "documents": [document]  # Must be a list containing one document
    }
...
def split_train_val_data(documents: List[Dict], config: LiLTConfig) -> Tuple[List[Dict], List[Dict]]:
    ...
    if config.use_overlapping_split:
        ...
        logger.info(f"Overlapping split: {len(train_docs)} training, {len(val_docs)} validation (with overlap)")
        ...
        try:
            ...
            if config.use_augmentation:
                try:
                    ...
                    if words and boxes:  # Only augment if we have valid words and boxes
                        ...
                        if aug_doc_info and len(aug_doc_info.get("entities", [])) > 0:
                            ...
                            logger.warning(f"Augmented document {input_path.name} has 0 entities - skipping")
                        # Cleanup temporary augmented image
...
def copy_image_and_json(doc_info: Dict[str, Any], output_dir: Path, config: LiLTConfig) -> bool:
    try:
        ...
        # Create clean JSON without temporary paths
        ...
        logger.exception(f"Error copying files for {doc_info.get('image_path')}: {e}")
        return False
def process_image_file(file_path: Path, output_dir: Path, config: LiLTConfig, model, tokenizer, device: torch.device) -> bool:
    ...
    if imghdr.what(file_path) is None:
        logger.warning(f"Skipping {file_path.name}: not a recognized image file or corrupted")
        return False
    ...
    if doc_info:
        ...
        try:
            ...
            logger.exception(f"Error saving JSON for {file_path.name}: {e}")
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Generate training/validation data using LiLT model")
    ...
    parser.add_argument("--max_entities", type=int, default=350)  # FIXED: Default set to 350
    ...
    logger.info(f"Training documents copied: {train_success}/{len(train_docs)}")
    logger.info(f"Validation documents copied: {val_success}/{len(val_docs)}")
